    ...
    <nav className='mb-5'>
      <div className="w-full flex justify-between items-center px-0 py-5 lg:px-12 lg:py-6">
        ...
        <div className="hidden space-x-2 w-1/3 text-base font-Instrument font-normal text-light-white
          ...
          {links.map((link, i) => (
            <Link key={i} to={link.path}>{link.name}</Link>
          ))}
        </div>
        ...
        <div className='lg:hidden'>
          <img src={isMobileMenuOpen ? equis : hambuger} alt="hamburger icon"  width={24} height={24} className='hover:cursor-pointer'
      ...
      <div className={`lg:hidden w-full overflow-hidden transition-all duration-300 ease-in-out
        ...
        <div className="flex flex-col space-y-6 justify-center items-center text-base font-Instrument font-normal text-light-white">
          {links.map((link, i) => (
            <Link key={i} to={link.path}>{link.name}</Link>
          ))}
        </div>
